...
public class UserDao {
     ...
     *      Notice! get과 get의 대상은 카멜케이스로 작성됩니다. ex) item -> getItem, pwd -> getPassword, email -> getEmail, userIdx -> getUserIdx
    ...
    public int createUser(PostUserReq postUserReq) {
        String createUserQuery = "insert into User (email, pwd, userName, phoneNum) VALUES (?,?,?,?)"; // 실행될 동적 쿼리문
        Object[] createUserParams = new Object[]{postUserReq.getEmail(), postUserReq.getPwd(), postUserReq.getUserName(), postUserReq.getPhoneNum()}; // 동적 쿼리의 ?부분에 주입될 값
        this.jdbcTemplate.update(createUserQuery, createUserParams);
        // email -> postUserReq.getEmail(), pwd -> postUserReq.getPassword(), nickname -> postUserReq.getNickname() 로 매핑(대응)시킨다음 쿼리문을 실행한다.
        // 즉 DB의 User Table에 (email, pwd, nickname)값을 가지는 유저 데이터를 삽입(생성)한다.
    ...
    public int modifyUserName(PatchUserReq patchUserReq) {
        String modifyUserNameQuery = "update User set userName = ? where userID = ? "; // 해당 userIdx를 만족하는 User를 해당 nickname으로 변경한다.
        Object[] modifyUserNameParams = new Object[]{patchUserReq.getUserName(), patchUserReq.getUserID()}; // 주입될 값들(nickname, userIdx) 순
    ...
    public User getPwd(PostLoginReq postLoginReq) {
        String getPwdQuery = "select userID, pwd,email,userName, phoneNum from User where email = ?"; // 해당 email을 만족하는 User의 정보들을 조회한다.
